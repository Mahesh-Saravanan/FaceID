...
def plot_loss(loss_bin, epoch, log_folder="log"):
    ...
    save_path = os.path.join(log_folder, f"loss_epoch_{epoch}.png")
...
def train(model, train_loader, val_loader, criterion, optimizer, device, num_epochs, save_path=None,plot_loss_flag = True):
    ...
    for epoch in range(num_epochs):
        ...
        if config.SaveInterval%(epoch+1):
            if save_path:
                torch.save(model.encoder.state_dict(), os.path.join(save_path, f"encoder_at_Epoch{epoch}.pth"))
                torch.save(model.decoder.state_dict(), os.path.join(save_path, f"decoder_at_Epoch{epoch}.pth"))
